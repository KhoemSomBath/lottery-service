...
public class TotalDailyReportService extends BaseServiceIP {
    ...
    public StructureRS getReport() {
        ...
        try {
            ...
            List<SummeryDailyEntity> summeryDailyEntities = summeryDailyRP.findAllByLotteryUserCodeIn(reportRQ.getFilterByLotteryType(), userCodes, reportRQ.getFilterByStartDate(), reportRQ.getFilterByEndDate());
            List<SettlementItemsEntity> settlementItemsEntities = settlementItemRP.listing(reportRQ.getFilterByLotteryType(), userCodes, reportRQ.getFilterByStartDate(), reportRQ.getFilterByEndDate());
            Map<String, CurrencyRS> oldBalanceMap = oldBalanceUtility.getUserOldBalanceMap(reportRQ.getFilterByLotteryType(), userCodes, reportRQ.getFilterByEndDate());
    ...
    private StructureRS getSelectedUserReport(TotalReportRQ reportRQ, UserToken userToken) {
        ...
        List<SummeryDailyEntity> summeryDailyEntities = summeryDailyRP.findAllByLotteryUserCodeIn(reportRQ.getFilterByLotteryType(), List.of(userEntity.getCode()), reportRQ.getFilterByStartDate(), reportRQ.getFilterByEndDate());
        List<SettlementItemsEntity> settlementItemsEntities = settlementItemRP.listing(reportRQ.getFilterByLotteryType(), List.of(userEntity.getCode()), reportRQ.getFilterByStartDate(), reportRQ.getFilterByEndDate());
        ...
        for (Date date = start.getTime(); start.before(end); date = start.getTime()) {
            ...
            SummeryDailyEntity summeryDailyEntity = summeryDailyEntities.stream().filter(it -> currentDate.equals(generalUtility.formatDateYYYYMMDD(it.getIssuedAt()))).findFirst().orElse(null);
            if (summeryDailyEntity != null) {
                SummaryRS summaryRS = summaryDailyUtility.getSummaryRSFromEntity(summeryDailyEntity);
                setUpDataFromSummeryDaily(summaryRS, item);
                summaryDaily.setAmountKhr(summeryDailyEntity.getTotalTurnoverKhr().subtract(summeryDailyEntity.getTotalRewardKhr()));
                summaryDaily.setAmountUsd(summeryDailyEntity.getTotalTurnoverUsd().subtract(summeryDailyEntity.getTotalRewardUsd()));

            }

            Map<String, CurrencyRS> oldBalanceMap = oldBalanceUtility.getUserOldBalanceMap(reportRQ.getFilterByLotteryType(), List.of(userEntity.getCode()), currentDate);
