...
import com.hacknovation.systemservice.v1_0_0.io.entity.th.THDrawingEntity;
...
public interface THDrawingRP extends JpaRepository<THDrawingEntity, Long> {
    THDrawingEntity findByCode(String code);


    @Query(value = "SELECT * FROM th_drawing where date(resulted_post_at) = :date", nativeQuery = true)
    Page<THDrawingEntity> getDrawByDate(String date, Pageable pageable);

    Boolean existsByCode(String code);

    @Query(value = "SELECT * FROM th_drawing  ORDER BY id DESC LIMIT 1", nativeQuery = true)
    THDrawingEntity lastDraw();

    @Query(value = "SELECT * FROM th_drawing WHERE `is_recent` = 1 ORDER BY id ASC LIMIT 1", nativeQuery = true)
    THDrawingEntity recentDrawing();

    @Query(value = "SELECT * FROM th_drawing WHERE `code` = :drawCode LIMIT 1", nativeQuery = true)
    THDrawingEntity getOriginalDraw(String drawCode);

    @Transactional
    @Modifying
    @Query(value = "UPDATE th_drawing SET is_recent = :isRecent, is_set_win = 0 WHERE code = :drawCode", nativeQuery = true)
    void updateIsRecent(String drawCode, boolean isRecent);

    @Transactional
    @Modifying
    @Query(value = "UPDATE th_drawing SET is_set_win = 1 WHERE code = :drawCode", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE th_drawing SET `status` = 'AWARDED' WHERE `code` = :drawCode ", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "DELETE FROM th_drawing_items WHERE drawing_id = (SELECT id FROM th_drawing WHERE code = :drawCode)", nativeQuery = true)
    void resetDrawingItems(String drawCode);

    @Transactional
    @Modifying
    @Query(value = "INSERT INTO th_drawing SELECT * FROM th_drawing WHERE code = :drawCode", nativeQuery = true)
    void saveDrawing(String drawCode);

    @Transactional
    @Modifying
    @Query(value = "INSERT INTO th_drawing_items SELECT * FROM th_drawing_items WHERE drawing_id = (SELECT id FROM th_drawing WHERE code = :drawCode)", nativeQuery = true)
