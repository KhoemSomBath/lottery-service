SELECT sum(if(oi.cc = 'khr', oi.total_amount, 0))                                                             betAmountKhr,
       ...
       sum(if(oi.cc = 'khr', if(oiw.id is not null, oi.total_amount * oiw.wq * oi.member_rebate_rate, 0), 0)) winAmountKhr,
...
FROM vnone_temp_orders o
         LEFT JOIN vnone_temp_order_items oi ON o.id = oi.oi
         LEFT JOIN vnone_temp_win_order_items oiw ON oi.id = oiw.oii
         LEFT JOIN users u ON u.code = o.uc
WHERE IF(:drawCode <> 'all', o.dc = :drawCode, TRUE)
  AND IF(lower(:isCancel) <> 'false', o.status = 0 , o.status = 1)
